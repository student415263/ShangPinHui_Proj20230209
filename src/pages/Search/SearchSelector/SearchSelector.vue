<template>
  <div class="clearfix selector">
      ...
      <div class="value logos">
        <ul class="logo-list">
          <!-- <li
            ...
            @click="trademarkHandle"
            :key="trademark.tmId">           -->
          <li @click="trademarkHandle(a)">
          1111122222</li>
          <li @click="trademarkHandle(a)">TCL</li>
          <li @click="trademarkHandle(a)">长虹（CHANGHONG）</li>
          <li>飞利浦（PHILIPS）</li>
          <li>风行电视</li>
    ...
    <!-- //--属性，平台的首卖属性，应该遍历出来，但是遍历的时候有gub，就暂时用着死数据 -->
    <div class="type-wrap">
      <div class="fl key">网络制式</div>
      <div class="fl value">
        <ul class="type-list">
          <li @click = 'attrInfo(Element)'>
            <a>GSM（移动/联通2G）</a>
          </li>
          <li>
            <a>电信2G</a>
...
import { mapGetters } from 'vuex';
  export default {
    name: 'ComSearchSelector',

    computed:{
      //在模板中用for-in去使用trademarkList的时候，会报错。故，不用了。
      ...mapGetters("trademarkList", "attrsList")
    },

    methods:{
      trademarkHandle(a){
        // this.$router.push('name': '/Route1Search')//XXXXX
        // a is Element
        this.$emit('trandemarkInfo', a)
      },
      //e is Element, 按理说应该有两个参数，一个参数是遍历平台属性时的属性，另一个参数是遍历平台属性的具体值
      attrInfo(e){
        this.$emit('attrInfo', e)
      }
    },
